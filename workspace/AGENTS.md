...
5. Read `memory/memory-config.json` — check `shardsEnabled`. If `true`, read the shard files listed in `shardPaths`. If `false`, skip them entirely (rely on Hippocampus facts in MEMORY.md instead).
6. **If context is missing or you feel clueless:** Read your most recent session backup (`agents/main/sessions/*.jsonl` — sort by LastWriteTime, read the latest). This is your true memory — every tool call, every message, every decision. It always exists even when daily logs don't.
