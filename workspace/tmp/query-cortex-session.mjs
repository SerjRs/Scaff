...
// Get all session messages from today, webchat channel, ordered by time
const msgs = db.prepare(`
  SELECT id, role, channel, sender_id, sender_name, content, timestamp, shard_id, issuer
  FROM cortex_session
  WHERE timestamp >= '2026-03-14T11:55:00'
...
for (const m of msgs) {
    ...
    const content = m.content && m.content.length > 500 ? m.content.substring(0, 500) + '...' : m.content;
