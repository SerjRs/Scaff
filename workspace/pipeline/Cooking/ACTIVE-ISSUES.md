...
| 37 | Async task results dropped on WhatsApp — threadId lost in transit | `threadId` (WhatsApp chatId) threaded through execution pipeline but lost at ops-trigger envelope creation. WhatsApp adapter: `chatId = null → response dropped`. User never gets the answer. | New `cortex_task_dispatch` table. Cortex stores full dispatch context at spawn time, restores at result time. Pipeline is stateless. PR #6 merged. | ✅ Fixed |
...
| 38 | **Thinking block + signature leak to WhatsApp** | Raw JSON `[{"type":"thinking","thinking":"...","thinkingSignature":"Etg..."}]` sent to users on WhatsApp. The cryptographic thinking signature from Anthropic API is visible. Observed 7 times in a single Cortex conversation (2026-03-14). | Trace where `parseResponse()` or `appendResponse()` handles thinking blocks. They must be stripped before storage/delivery. The `thinkingSignature` should never leave the LLM layer. |
| 39 | **Cortex uses sessions_spawn for local file operations** | Cortex called `sessions_spawn` to run `mv` (move a folder from Cooking → InProgress). It has `move_file` as a sync tool — the operation should be instant, not dispatched through the Router. Wasted ~30s of executor time + an API call. | Add guidance to Cortex system prompt: sync tools (`read_file`, `write_file`, `move_file`, `delete_file`, `code_search`, `memory_query`) are for local operations. `sessions_spawn` is for work that requires an executor (coding, research, complex multi-step tasks). Never spawn for file moves, reads, or writes. |
| 40 | **Cortex doesn't complete pipeline end-to-end** | Cortex executed task 002: created CLAUDE.md + STATE.md, moved to InProgress, spawned executor, executor completed and created PR #7. But Cortex never merged the PR and never moved to Done. Left the task dangling. | Add pipeline completion checklist to Cortex's system prompt or CLAUDE.md template: after executor reports success → verify build → merge PR → move folder to Done → confirm to user. |
| 41 | **Cortex false claims / confused self-narration** | (a) Said "files are getting truncated" when they weren't — confused tool display limit with data loss. (b) Said "Task 002 is already complete — a previous executor had already implemented it" when this was the first and only execution. | Likely a context/shard issue — Cortex loses track of its own actions. May need clearer self-awareness in system prompt, or better structured storage of its own decisions. |
...
|---|------|----------------------|-------|
| D1 | **`cortex_pending_ops` table + System Floor injection** | `cortex_pending_ops` table **dropped and replaced** by `cortex_task_dispatch` (PR #6). Results still written to `cortex_session` via `appendTaskResult()`, but dispatch context (channel, counterpart, shard) now stored in `cortex_task_dispatch` for correlation. | Simpler — no dual-path. Result goes straight to Foreground. Dispatch context enables correct reply routing. |
